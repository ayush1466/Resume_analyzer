...
const AnalysisResult = ({ result }) => {
  const renderSection = (title, items, icon, colorClass, bgClass, borderClass) => (
    <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-xl border border-gray-700 shadow-xl p-6 hover:shadow-2xl transition-all duration-300 hover:scale-[1.02]">
      <div className="flex items-center gap-3 mb-5 pb-4 border-b border-gray-700">
        ...
        <span className={`ml-auto text-xs font-semibold px-3 py-1 rounded-full ${bgClass} ${colorClass}`}>
          {items.length}
        </span>
      </div>
      <ul className="space-y-3">
        {items.map((item, idx) => (
          <li key={idx} className="flex items-start gap-3 text-gray-300 group">
            <span className={`${colorClass} mt-1 opacity-70 group-hover:opacity-100 transition-opacity`}>
              •
            </span>
    ...
    <div className="w-full max-w-6xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-gradient-to-br from-emerald-500/10 to-green-600/10 border border-emerald-500/30 rounded-xl p-4">
          ...
          <div className="text-3xl font-bold text-emerald-400">{result.strengths.length}</div>
        </div>
        ...
        <div className="bg-gradient-to-br from-yellow-500/10 to-orange-600/10 border border-yellow-500/30 rounded-xl p-4">
          ...
          <div className="text-3xl font-bold text-yellow-400">{result.improvements.length}</div>
        </div>
        ...
        <div className="bg-gradient-to-br from-red-500/10 to-pink-600/10 border border-red-500/30 rounded-xl p-4">
          ...
          <div className="text-3xl font-bold text-red-400">{result.missingKeywords.length}</div>
        </div>
        ...
        <div className="bg-gradient-to-br from-cyan-500/10 to-blue-600/10 border border-cyan-500/30 rounded-xl p-4">
          ...
          <div className="text-3xl font-bold text-cyan-400">{result.suggestions.length}</div>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6 mt-8">
        {result.strengths.length > 0 && renderSection(
          'Strengths',
          result.strengths,
          <CheckCircle className="w-5 h-5 text-emerald-400" />,
          'text-emerald-400',
        ...
        {result.improvements.length > 0 && renderSection(
        ...
        {result.missingKeywords.length > 0 && renderSection(
          'Missing Keywords',
        ...
        {result.suggestions.length > 0 && renderSection(
      ...
      <div className="flex gap-4 justify-center mt-8 pt-8 border-t border-gray-800">
        <button className="px-6 py-3 bg-gradient-to-r from-cyan-500 to-blue-600 text-white rounded-xl font-medium hover:from-cyan-400 hover:to-blue-500 transition-all shadow-lg shadow-cyan-500/30 hover:shadow-cyan-500/50">
          Download Report
        </button>
